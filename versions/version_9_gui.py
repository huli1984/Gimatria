...
class gimatria_gui(QMainWindow):
    ...
    def printAppResult(self, vals, lines, searched_word_indices):
        ...
        #aggiunge le values lines al list output (la finestra centrale nel Qt5 window
        #self.listOutput.addItems(lines)

        #nell'index mettere le coordinate (for loop? list cohomprension?) corrispondenti al ritrovamento della parola identica a quella posta in ricerca
        #così da evidenziarla nell'output
        ...
        #raw -> cerca indicizzazione data parola con colonne e rows

        #index = QtCore.QPersistentModelIndex(
        #    self.listOutput.model().index(7, 2))
        #print(index.column(), index.row(), index.data())

        #cerca indicizzazione parole in accordo con searched_word_indices
        #for item in searched_word_indices:
        #    index = QtCore.QPersistentModelIndex(self.listOutput.model().index(item, 2))
        #    print(index.column(), index.row(), index.data(), "print indices")
